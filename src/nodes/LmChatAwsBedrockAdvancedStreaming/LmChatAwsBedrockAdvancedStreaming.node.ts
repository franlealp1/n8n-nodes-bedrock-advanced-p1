/**
 * N8N node definition for AWS Bedrock Chat Model Streaming (P1).
 *
 * Sibling of LmChatAwsBedrockAdvanced — shares the same descriptor surface
 * (authType router, model pickers, cache options) and inherits the Patched
 * behavior (sanitize + cache-point inject + metrics) via its subclass
 * ChatAwsBedrockAdvancedStreaming (which extends PatchedChatBedrockConverse).
 *
 * Delta vs the non-streaming sibling: new "Streaming" collection with 5 fields
 * (callbackUrl, sessionId, authHeaderValue, batchIntervalMs, maxBatchChars) and
 * the constructor wires them into the subclass. Empty callbackUrl yields
 * byte-identical output to the non-streaming Advanced node.
 */
...
class LmChatAwsBedrockAdvancedStreamingP1 implements INodeType {
	description: INodeTypeDescription = {
		displayName: 'AWS Bedrock Chat Model Streaming (P1)',

		name: 'lmChatAwsBedrockAdvancedStreamingP1',
		icon: 'file:bedrock.svg',
		group: ['transform'],
		version: [1],
		description: 'AWS Bedrock Advanced (P1) with token-level streaming callback — subclass of PatchedChatBedrockConverse; sends text deltas via HTTP POST during generation.',
		defaults: {
			name: 'AWS Bedrock Chat Model Streaming (P1)',
		},
		codex: {
			categories: ['AI'],
			subcategories: {
				...
				options: [
					{
						...
						type: 'string',
						default: '',
						description: 'HTTP endpoint to POST {streamId, seq, delta, done} during generation. Empty = no streaming (byte-identical to non-streaming Advanced node).',
					},
					{
	...
	async supplyData(this: ISupplyDataFunctions, itemIndex: number): Promise<SupplyData> {
		...
		// Streaming subclass of PatchedChatBedrockConverse: inherits sanitize +
		// cache-inject + cache-metrics; overrides _streamResponseChunks to
		// side-channel deltas via HTTP callback (no-op when Callback URL empty).
...
// Export as LmChatAwsBedrockAdvancedStreaming for N8N loader compatibility
export { LmChatAwsBedrockAdvancedStreamingP1 as LmChatAwsBedrockAdvancedStreaming };

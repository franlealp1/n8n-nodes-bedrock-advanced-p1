/**
 * N8N node definition for Bedrock Claude Streaming (P1).
 *
 * Sibling of the base `Bedrock Claude (P1)` node. Instantiates the subclass
 * `ChatBedrockClaudeStreaming`, which overrides `_streamResponseChunks` to POST
 * text deltas to a configurable callback URL during generation. When the callback
 * URL is empty, the node behaves byte-identically to the base node.
 */
...
import type { AnthropicToolEntry } from '../LmChatBedrockClaude/toolConverter';
import {
	createWebSearchTool,
...
} from '../LmChatBedrockClaude/toolConverter';

// Class name uses P1 suffix internally; exported as LmChatBedrockClaudeStreaming
// for N8N loader compatibility (file name must match export name).
class LmChatBedrockClaudeStreamingP1 implements INodeType {
	description: INodeTypeDescription = {
		displayName: 'Bedrock Claude Streaming (P1)',
		name: 'lmChatBedrockClaudeStreamingP1',
		icon: 'file:bedrock-claude.svg',
		group: ['transform'],
		version: [1],
		description: 'Claude on AWS Bedrock with token-level streaming callback — subclass of Bedrock Claude (P1); sends text deltas via HTTP POST during generation.',
		defaults: {
			name: 'Bedrock Claude Streaming (P1)',
		},
		codex: {
			categories: ['AI'],
			subcategories: {
		...
		properties: [
			...
			},
			{
				...
				description: 'Configure the HTTP callback that receives text deltas during generation. Leave Callback URL empty to disable streaming and behave identically to Bedrock Claude (P1).',
				type: 'collection',
				default: {},
				options: [
					{
						...
						type: 'string',
						default: '',
						description: 'HTTP endpoint to POST text deltas to during generation. Empty = no streaming callback.',
					},
					{
						...
						type: 'string',
						default: '',
						description: 'Correlation ID propagated into every POST (streamId).',
					},
					{
						...
						description: 'Value of the x-webhook-auth header on each POST. Leave empty for no auth.',
					},
					{
						...
						type: 'number',
						default: 60,
						description: 'Flush buffered deltas every N ms. Set to 0 to disable the interval (flush on chars only).',
					},
					{
						...
						type: 'number',
						default: 120,
						description: 'Flush immediately when buffer reaches N characters.',
					},
				],
			},
		],
	...
	async supplyData(this: ISupplyDataFunctions, itemIndex: number): Promise<SupplyData> {
		...
		if (options.enableDebugLogs) {
			logger.info(`[BedrockClaudeStreaming] Model: ${modelName}`);
			logger.info(`[BedrockClaudeStreaming] Caching: ${options.enableCaching ?? false}`);
			logger.info(`[BedrockClaudeStreaming] Built-in tools: ${builtInTools.map(t => ('type' in t ? t.type : t.name)).join(', ') || 'none'}`);
			logger.info(`[BedrockClaudeStreaming] Callback URL: ${streaming.callbackUrl ? 'set' : 'empty (no streaming)'}`);
		}
...
// Export as LmChatBedrockClaudeStreaming for N8N loader compatibility
// (N8N expects export name to match file name)
export { LmChatBedrockClaudeStreamingP1 as LmChatBedrockClaudeStreaming };
